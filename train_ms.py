...
def run(rank, n_gpus, hps):
    ...
    # ── OVERSAMPLING: pass per-sample weights to sampler ─────────────────
    # Weights are defined per speaker in data_utils.py → TextAudioSpeakerLoader
    # Adjust speaker_weights dict in data_utils._filter() to tune oversampling
    if rank == 0:
        weights = train_dataset.get_weights()
        speaker_ids = [item[1] for item in train_dataset.audiopaths_sid_text]
        from collections import Counter
        counts = Counter(speaker_ids)
        print("\n── Speaker distribution after oversampling weights ──")
        for sid in sorted(counts.keys()):
            sid_weights = [w for w, s in zip(weights, speaker_ids) if s == sid]
            effective = sum(sid_weights)
            print(f"  Speaker {sid:>3s}: {counts[sid]:>6d} samples  "
                  f"weight={sid_weights[0]:.1f}  "
    ...
    train_sampler = DistributedBucketSampler(
        ...
        [32, 300, 400, 500, 600, 700, 800, 1000, 1200],
        num_replicas=n_gpus,
        rank=rank,
        shuffle=True,
        weights=train_dataset.get_weights(),  # ← ADDED: oversampling weights
    )
